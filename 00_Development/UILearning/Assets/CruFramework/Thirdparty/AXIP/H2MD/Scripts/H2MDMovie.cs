...
public class H2MDMovie{
	...
	public Texture2D Open(String path){
		...
		status=H2MDDecoder.h2mdOpenStreamFile(m_decoder,path);
		if(status!=H2MDDecoder.H2MDDEC_STATUS_SUCCESS){
			return null;
		}
	...
	public Texture2D OpenEx(IntPtr fopen_args,H2MDDecoder.h2mdDecoderCallback callback){
		int status;
		...
		status=H2MDDecoder.h2mdOpenStreamEx(m_decoder,fopen_args,callback,H2MDDecoder.H2MDDEC_CALLBACK_VERSION);
		if(status!=H2MDDecoder.H2MDDEC_STATUS_SUCCESS){
			return null;
		}
	...
	public bool SwapMem(byte [] code){
		int status;

		/* 既存のデコーダインスタンスを開放 */

		Texture2D backup_tex=m_tex;	/* テクスチャは保持する */
		H2MDDecoder.h2mdMovieInfo backup_movie_info=m_movie_info;
		Dispose();
		m_tex=backup_tex;	/* テクスチャを復元する */
		...
		status=H2MDDecoder.h2mdGetMovieInfo(m_decoder,m_movie_info,H2MDDecoder.H2MDDEC_MOVIE_INFO_VERSION);
		if(status!=H2MDDecoder.H2MDDEC_STATUS_SUCCESS){
			return false;
		}

		if(backup_movie_info.width!=m_movie_info.width || backup_movie_info.height!=m_movie_info.height){
			return false;
		}
	...
	public int GetImage(){
		return GetImageCore(false);
	}

	public int GetImageFlip(){
		return GetImageCore(true);
	}

	private int GetImageCore(bool flip){
		if(m_decoder==H2MDDecoder.DECODER_NULL){
			return H2MDDecoder.H2MDDEC_STATUS_INVALID_STATE;
		}
		...
		lock(lock_object){
			...
			/* WebGL用 */
				...
				if(flip){
					return H2MDDecoder.h2mdGetImageFlip(m_decoder, (IntPtr)(m_tex.GetNativeTextureID()), 0 , 0);
				}else{
					return H2MDDecoder.h2mdGetImage(m_decoder, (IntPtr)(m_tex.GetNativeTextureID()), 0 , 0);
				}
			#else
			...
			int status;
			if(flip){
				status=H2MDDecoder.h2mdGetImageFlip(m_decoder,m_rgbquad_image,(int)(m_movie_info.height*image_stride),image_stride);
			}else{
				status=H2MDDecoder.h2mdGetImage(m_decoder,m_rgbquad_image,(int)(m_movie_info.height*image_stride),image_stride);
			}
	...
	public void Dispose(){
		...
		lock(lock_object){
			...
			if (m_image_handle.IsAllocated){
				m_image_handle.Free();
			}
			m_image_color32=null;
			m_tex=null;
			m_rgbquad_image = IntPtr.Zero;
