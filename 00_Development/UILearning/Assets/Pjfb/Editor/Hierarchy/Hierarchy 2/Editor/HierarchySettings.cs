...
namespace Hierarchy2
{
    ...
    internal class HierarchySettings : ScriptableObject
    {
        ...
        public int componentRightSpacing = 0;
        ...
        static SettingsProvider UIElementSettingsProvider()
        {
            var provider = new SettingsProvider("Project/Hierarchy", SettingsScope.Project)
            {
                ...
                activateHandler = (searchContext, rootElement) =>
                {
                    ...
                    var isEnabled = new Toggle("isEnabled");
                    isEnabled.value = settings.isEnabled;
                    isEnabled.RegisterValueChangedCallback((evt) =>
                    {
                        Undo.RecordObject(settings, "Change Settings");

                        settings.isEnabled = evt.newValue;
                        settings.OnSettingsChanged(nameof(settings.isEnabled));
                    });
                    isEnabled.StyleMarginLeft(CONTENT_MARGIN_LEFT);
                    verticalLayout.Add(isEnabled);
                    ...
                    var componentRightSpacing = new IntegerField();
                    componentRightSpacing.label = "Component Right Spacing";
                    componentRightSpacing.value = settings.componentRightSpacing;
                    componentRightSpacing.StyleMarginLeft(CONTENT_MARGIN_LEFT);
                    componentRightSpacing.RegisterValueChangedCallback((evt) =>
                    {
                        Undo.RecordObject(settings, "Change Settings");

                        settings.componentRightSpacing = evt.newValue;
                        settings.OnSettingsChanged(nameof(settings.componentRightSpacing));
                    });
                    verticalLayout.Add(componentRightSpacing);
